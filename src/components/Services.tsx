...
import { SERVICE_ICONS, ArrowIcon, CheckIcon, PlusIcon, WhatsAppIcon } from "./icons";
import { Eyebrow, MaskLines } from "./Reveal";

export default function Services() {
  const [openIdx, setOpenIdx] = useState(0);
  const [hoverIdx, setHoverIdx] = useState<number | null>(null);
  const active = SERVICES[hoverIdx ?? (openIdx >= 0 ? openIdx : 0)];
      ...
      <div className="relative mx-auto max-w-7xl px-6">
        <div className="grid gap-6 lg:grid-cols-12 lg:items-end">
          ...
          <p className="max-w-sm text-[15.5px] leading-relaxed text-ink-600 lg:col-span-4">
            Cinco áreas que cubren la vida completa de tu empresa ante el SII, la municipalidad y
            la TGR. Recorre la lista: cada línea es una partida de nuestro libro.
          </p>
        </div>
              ...
              >
                <div className="flex items-start justify-between">
                  <span className="tabular font-display text-7xl font-extrabold leading-none text-brass-400">
                    {String((openIdx >= 0 ? openIdx : 0) + 1).padStart(2, "0")}
                  </span>
                ...
                <div className="mt-6 h-[3px] w-full bg-ink-700">
                  <div
                    ...
                    style={{
                      width: `${(((openIdx >= 0 ? openIdx : 0) + 1) / SERVICES.length) * 100}%`,
                    }}
                  />
                </div>
                  ...
                  <li key={s.id} className="border-t border-ink-900/15 last:border-b">
                    ...
                    >
                      <div className="overflow-hidden">
                        <div className="grid gap-6 pb-8 pl-[55px] pr-2 sm:grid-cols-[1fr_270px] sm:pl-[77px]">
                          <div>
                          ...
                          <div className="flex flex-col items-start justify-end gap-3">
                            <a
                              href="#contacto"
                              className="group/link inline-flex items-center gap-2.5 border border-ink-900 px-5 py-3.5 font-mono text-[11px] font-semibold uppercase leading-snug tracking-[0.12em] text-ink-900 transition-all duration-300 hover:bg-ink-900 hover:text-brass-300"
                            >
                              {s.cta}
                              <ArrowIcon className="h-4 w-4 shrink-0 transition-transform duration-300 group-hover/link:translate-x-1" />
                            </a>
                            <a
                              href={waLink(s.wa)}
                              target="_blank"
                              ...
                              className="group/wa inline-flex items-center gap-2.5 border border-ink-900/25 px-5 py-3 font-mono text-[10.5px] uppercase tracking-[0.14em] text-ink-700 transition-all duration-300 hover:border-[#25d366] hover:bg-[#25d366] hover:text-ink-950"
                            >
                              <WhatsAppIcon className="h-4 w-4 text-[#1faa55] transition-colors duration-300 group-hover/wa:text-ink-950" />
