...
const LS_KEY = "audicontab_user_reviews_v2";
    ...
    <span className="inline-flex items-center gap-1" aria-label={`${value} de 5 estrellas`}>
      {[1, 2, 3, 4, 5].map((i) => (
        <svg
          key={i}
          viewBox="0 0 20 20"
          className={`${size} ${i <= value ? "text-brass-400" : "text-ink-900/15"}`}
          fill="currentColor"
          aria-hidden="true"
        >
          <path d={STAR_PATH} />
  ...
  return (
    <article
      className={`group relative flex h-full flex-col border bg-paper-50 p-6 transition-all duration-300 ${
        fresh
          ? "border-2 border-brass-400 shadow-[0_18px_44px_-20px_rgba(37,99,235,0.45)]"
          : "border-ink-900/15 hover:-translate-y-1.5 hover:border-brass-400/60 hover:shadow-[0_20px_44px_-22px_rgba(27,58,92,0.3)]"
      }`}
    >
      {fresh && (
        <span className="absolute -top-3 left-5 bg-brass-400 px-2.5 py-1 font-mono text-[9.5px] font-bold uppercase tracking-[0.2em] text-paper-50">
          Nueva
        </span>
      )}
      ...
      <footer className="mt-5 flex items-center gap-3 border-t border-ink-900/10 pt-4">
        <span className="flex h-10 w-10 shrink-0 items-center justify-center border border-brass-500/40 bg-brass-400/15 font-display text-sm font-extrabold text-brass-500">
          {initials}
        </span>
        <span>
...
export default function Reviews() {
  ...
  return (
    <section id="clientes" className="relative overflow-hidden bg-paper-100 py-20 md:py-28">
      ...
      <div
        aria-hidden="true"
        className="absolute inset-0 bg-[radial-gradient(680px_380px_at_0%_0%,rgba(37,99,235,0.12),transparent_60%)]"
      />
      ...
      <div className="relative mx-auto max-w-7xl px-6">
        ...
        <div className="mt-14 grid gap-10 lg:grid-cols-12 lg:gap-12">
          ...
          <div className="space-y-6 lg:col-span-5">
            {reviews.length > 0 && (
              <div ref={sumRef} className="border-l-4 border-brass-400 bg-ink-950 px-8 py-7 text-paper-50">
                <div className="flex items-end gap-4">
                  <p className="tabular font-display text-5xl font-extrabold leading-none text-paper-50">
                    {avgFmt.format(avg)}
                  </p>
                  <div className="pb-1">
                    <Stars value={Math.round(avg)} />
                    <p className="mt-1.5 font-mono text-[10px] uppercase tracking-[0.22em] text-mist-400">
            ...
            >
              ...
              <div className="relative">
                ...
                <div className="mt-6 space-y-5">
                          ...
                          <svg
                            ...
                            className={`h-8 w-8 transition-colors duration-200 ${
                              i <= (hoverStar || form.rating)
                                ? "text-brass-400 drop-shadow-[0_0_8px_rgba(37,99,235,0.45)]"
                                : "text-paper-50/20"
                            }`}
                            fill="currentColor"
                            aria-hidden="true"
                  ...
                  <button
                    type="submit"
                    className="group inline-flex w-full items-center justify-center gap-3 bg-brass-500 px-6 py-4 font-mono text-[12.5px] font-semibold uppercase tracking-[0.16em] text-paper-50 shadow-md transition-all duration-300 hover:bg-brass-400 hover:shadow-[0_14px_32px_-12px_rgba(37,99,235,0.6)] active:scale-[0.99]"
                  >
                    Publicar reseña
          ...
          <div className="lg:col-span-7">
            {reviews.length === 0 ? (
              <div className="flex h-full flex-col items-center justify-center border-2 border-dashed border-ink-900/20 bg-paper-50/60 p-12 text-center">
                <div className="flex h-16 w-16 items-center justify-center rounded-full bg-brass-400/10 text-brass-500">
                  <Stars value={5} size="h-6 w-6" />
                </div>
                <h4 className="mt-6 font-display text-2xl font-bold text-ink-900">
                  Aún no hay reseñas publicadas
                </h4>
                <p className="mt-2.5 max-w-md text-[15px] leading-relaxed text-ink-600">
                  Sé el primero en compartir tu testimonio sobre nuestros servicios contables y tributarios utilizando el formulario a la izquierda.
                </p>
              </div>
            ) : (
              <div>
